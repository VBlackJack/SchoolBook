# Module 10 - Le monde anglophone / The English-Speaking World

!!! info "Objectifs du module"
    À la fin de ce module, tu sauras :

    - 🎯 Situer les principaux pays anglophones
    - 🎯 Connaître les symboles du Royaume-Uni et des USA
    - 🎯 Découvrir les fêtes et traditions
    - 🎯 Comparer les cultures anglophones et française

    **Durée estimée : 2-3 heures**

---

## 🎮 In Real Life: English culture everywhere!

!!! tip "La culture anglophone dans ta vie"

    **Ce que tu connais déjà :**

    | Catégorie | Exemples anglophones |
    |-----------|---------------------|
    | **Films** | Marvel, Disney, Harry Potter, Star Wars |
    | **Musique** | Ed Sheeran, Taylor Swift, Billie Eilish |
    | **Jeux vidéo** | Fortnite (USA), Minecraft (Suède mais en anglais) |
    | **Sport** | NBA, NFL, Premier League |
    | **Séries** | Stranger Things, The Simpsons |

!!! example "Les fêtes anglophones"

    **Fêtes que tu vois dans les films/jeux :**

    | Fête | Date | Particularité |
    |------|------|---------------|
    | **Halloween** | 31 octobre | Costumes, trick or treat |
    | **Thanksgiving** | 4ème jeudi de novembre (USA) | Turkey (dinde), family |
    | **Christmas** | 25 décembre | Father Christmas / Santa Claus |
    | **St Patrick's Day** | 17 mars (Irlande) | Vert partout ! |

    **Dans Fortnite et autres jeux = événements spéciaux à ces dates !**

!!! warning "Différences UK vs USA"

    | Sujet | UK (British) | USA (American) |
    |-------|-------------|----------------|
    | **Football** | ⚽ Football | 🏈 Soccer (football = foot US) |
    | **Orthographe** | colour, favourite | color, favorite |
    | **Vocabulaire** | chips = frites | chips = chips, fries = frites |
    | **Monnaie** | Pound (£) | Dollar ($) |
    | **Conduite** | À gauche | À droite |

??? warning "⚠️ Pièges à éviter : culture anglophone"

    **Piège 1 : UK ≠ England**

    | Terme | Ce que c'est |
    |-------|-------------|
    | **UK** | England + Scotland + Wales + Northern Ireland |
    | **England** | Un seul des 4 pays du UK |
    | **Great Britain** | England + Scotland + Wales (sans N. Ireland) |

    ❌ "England" pour parler de tout le UK
    ✅ Utilise "**UK**" ou "**Britain**" pour l'ensemble

    ---

    **Piège 2 : Le vocabulaire UK vs USA**

    | UK | USA | Français |
    |----|-----|----------|
    | flat | apartment | appartement |
    | lift | elevator | ascenseur |
    | chips | fries | frites |
    | crisps | chips | chips (paquet) |
    | football | soccer | football ⚽ |

    **Attention au contexte (jeu US ou UK) !**

    ---

    **Piège 3 : Les dates américaines**

    | UK/France | USA |
    |-----------|-----|
    | jour/mois | **mois/jour** |
    | 5/6 = 5 juin | 5/6 = **6 mai** |

    **Independence Day = July 4th = 4/7 en France, 7/4 aux USA !**

    ---

    **Piège 4 : Les fêtes à ne pas confondre**

    | Fête | Pays | Date |
    |------|------|------|
    | **Thanksgiving** | USA seulement | 4ème jeudi de novembre |
    | **Guy Fawkes Night** | UK seulement | 5 novembre |
    | **Halloween** | USA + UK | 31 octobre |

    ❌ Thanksgiving n'existe pas au UK !

    ---

    **Piège 5 : La monnaie**

    | Pays | Monnaie | Symbole |
    |------|---------|---------|
    | UK | Pound/Livre | **£** |
    | USA | Dollar | **$** |
    | Irlande | Euro | € |

    ❌ "Dollars" au UK → ✅ "**Pounds**"

??? abstract "🔄 Autre façon de comprendre : UK vs USA"
    **Deux pays, deux versions de l'anglais !**

    Imagine deux versions d'un même jeu : la version UK et la version US. C'est le même jeu, mais avec quelques différences !

    ---

    **1. L'orthographe (spelling) :**

    Les Américains ont simplifié certains mots :

    | UK (complet) | USA (simplifié) |
    |--------------|-----------------|
    | colo**u**r | color |
    | favo**u**rite | favorite |
    | cent**re** | center |
    | theatre | theater |

    **Astuce : les jeux américains utilisent souvent la version US !**

    ---

    **2. Le vocabulaire (words) :**

    Attention, certains mots sont complètement différents :

    ```
    🇬🇧 UK          🇺🇸 USA         🇫🇷 Français
    ─────────────────────────────────────────
    flat           apartment      appartement
    lift           elevator       ascenseur
    biscuit        cookie         biscuit
    chips          fries          frites
    crisps         chips          chips (paquet)
    football       soccer         football
    rubbish        garbage        poubelle
    queue          line           file d'attente
    ```

    ---

    **3. Le piège des frites et chips :**

    ```
    🇬🇧 UK :
    - chips 🍟 = frites
    - crisps 🥔 = chips (paquet)

    🇺🇸 USA :
    - fries 🍟 = frites
    - chips 🥔 = chips (paquet)
    ```

    **Si tu demandes "chips" au UK, tu auras des frites !**

!!! example "La pop culture anglophone"

    **Ce que tout le monde connaît :**

    | Personnage/Série | Pays |
    |------------------|------|
    | **Harry Potter** | UK (Royaume-Uni) |
    | **Spider-Man** | USA |
    | **Sherlock Holmes** | UK |
    | **Mickey Mouse** | USA |
    | **The Beatles** | UK |
    | **Marvel/DC** | USA |
    | **James Bond** | UK |

    **L'anglais te donne accès à TOUT ça en version originale !**

!!! tip "Pourquoi apprendre l'anglais ?"

    **Statistiques :**

    - **1,5 milliard** de personnes parlent anglais
    - **60%** des sites web sont en anglais
    - **80%** des jeux vidéo sont créés en anglais
    - **Tous** les grands films sortent d'abord en anglais

    **Avec l'anglais, tu peux :**

    - Jouer aux jeux avant la traduction française
    - Comprendre les streams internationaux
    - Voyager partout dans le monde
    - Accéder à plus de contenus sur internet

---

## 1. Les pays anglophones

### 1.1 Où parle-t-on anglais ?

```
    ENGLISH-SPEAKING COUNTRIES

    🇬🇧 United Kingdom (UK)        Royaume-Uni
       ├─ England                  Angleterre
       ├─ Scotland                 Écosse
       ├─ Wales                    Pays de Galles
       └─ Northern Ireland         Irlande du Nord

    🇮🇪 Ireland                     Irlande
    🇺🇸 United States (USA)         États-Unis
    🇨🇦 Canada                      Canada
    🇦🇺 Australia                   Australie
    🇳🇿 New Zealand                 Nouvelle-Zélande
    🇿🇦 South Africa                Afrique du Sud
    🇮🇳 India                       Inde
```

### 1.2 Capitales et langues

| Pays | Capitale | Population |
|------|----------|------------|
| United Kingdom | London | 67 millions |
| United States | Washington D.C. | 330 millions |
| Canada | Ottawa | 38 millions |
| Australia | Canberra | 26 millions |
| Ireland | Dublin | 5 millions |

!!! tip "Fun fact"
    L'anglais est la langue officielle ou une des langues officielles dans plus de **60 pays** !

---

## 2. Le Royaume-Uni - The United Kingdom

### 2.1 Les quatre nations

```
    THE UNITED KINGDOM

    ┌────────────────────────────────────────┐
    │           UNITED KINGDOM               │
    │                                        │
    │   🏴󠁧󠁢󠁳󠁣󠁴󠁿 SCOTLAND                       │
    │   Capital: Edinburgh                   │
    │   Symbol: thistle (chardon)            │
    │                                        │
    │   🏴󠁧󠁢󠁥󠁮󠁧󠁿 ENGLAND                        │
    │   Capital: London                      │
    │   Symbol: rose (rose)                  │
    │                                        │
    │   🏴󠁧󠁢󠁷󠁬󠁳󠁿 WALES                          │
    │   Capital: Cardiff                     │
    │   Symbol: daffodil (jonquille)         │
    │                                        │
    │   🇬🇧 NORTHERN IRELAND                  │
    │   Capital: Belfast                     │
    │   Symbol: shamrock (trèfle)            │
    │                                        │
    └────────────────────────────────────────┘
```

### 2.2 Symboles britanniques

```
    BRITISH SYMBOLS

    👑 The Queen/King           Le/La monarque
    🏰 Buckingham Palace        Palais de Buckingham
    🎡 Big Ben                  Big Ben
    🌉 Tower Bridge             Tower Bridge
    🚌 Red double-decker bus    Bus rouge à deux étages
    📞 Red phone box            Cabine téléphonique rouge
    ☕ Tea                       Le thé
    🍟 Fish and chips           Poisson-frites
```

### 2.3 L'école en Angleterre

| En France | En Angleterre |
|-----------|---------------|
| Collège (6ème-3ème) | Secondary school (Year 7-11) |
| Uniforme : rare | Uniforme : obligatoire |
| Cours jusqu'à 16h-17h | Cours jusqu'à 15h30 |
| Pas de clubs | After-school clubs |

---

## 3. Les États-Unis - The United States

### 3.1 Informations générales

```
    THE UNITED STATES OF AMERICA

    🗺️ 50 états (states)
    🏛️ Capitale : Washington D.C.
    💵 Monnaie : Dollar ($)
    🗽 Symbole : Statue of Liberty

    VILLES PRINCIPALES :
    ─────────────────────
    🗽 New York          (la plus grande ville)
    🌴 Los Angeles       (Hollywood, cinéma)
    🌁 San Francisco     (Golden Gate Bridge)
    🎰 Las Vegas         (casinos)
    🌴 Miami             (plages, Floride)
```

### 3.2 Symboles américains

```
    AMERICAN SYMBOLS

    🗽 Statue of Liberty    Statue de la Liberté
    🦅 Bald eagle           Aigle à tête blanche
    🇺🇸 Stars and Stripes    Drapeau (50 étoiles, 13 bandes)
    🏈 American football    Football américain
    🍔 Hamburger            Le hamburger
    🎬 Hollywood            Hollywood
    🏀 Basketball           Basketball
```

---

## 4. Fêtes et traditions - Festivals and Traditions

### 4.1 Fêtes britanniques

| Fête | Date | Tradition |
|------|------|-----------|
| **Christmas** | 25 décembre | Christmas crackers, pudding |
| **Boxing Day** | 26 décembre | Soldes, visites familiales |
| **Easter** | Mars/Avril | Easter eggs, Easter Bunny |
| **Guy Fawkes Night** | 5 novembre | Feux d'artifice, bonfires |
| **St Patrick's Day** | 17 mars | Vert, trèfle (Irlande) |

### 4.2 Fêtes américaines

```
    AMERICAN HOLIDAYS

    🦃 THANKSGIVING (4ème jeudi de novembre)
       Repas en famille, dinde (turkey), pumpkin pie
       Pour remercier (to give thanks)

    🎃 HALLOWEEN (31 octobre)
       Trick or treat! (Des bonbons ou un sort !)
       Costumes, pumpkins, scary decorations

    🇺🇸 INDEPENDENCE DAY (4 juillet)
       Feux d'artifice, parades, barbecues
       Anniversaire de l'indépendance (1776)

    ❤️ VALENTINE'S DAY (14 février)
       Cartes, chocolats, roses
       Fête des amoureux
```

### 4.3 Halloween en détail

```
    HALLOWEEN VOCABULARY

    🎃 pumpkin              citrouille
    👻 ghost                fantôme
    🧙 witch                sorcière
    🦇 bat                  chauve-souris
    🕷️ spider               araignée
    💀 skeleton             squelette
    🧛 vampire              vampire
    🧟 zombie               zombie
    🍬 candy                bonbons
    🏚️ haunted house        maison hantée

    "Trick or treat!"
    (Des bonbons ou un sort !)
```

---

## 5. Différences culturelles

### 5.1 Système scolaire

```
    SCHOOL SYSTEMS

    FRANCE                      UK / USA
    ────────────────────────────────────────
    École primaire             Primary school
    (6-11 ans)                 (5-11 ans)

    Collège                    Secondary school /
    (11-15 ans)                Middle + High school

    Lycée                      Sixth Form /
    (15-18 ans)                High school

    Notes : /20                Notes : A, B, C, D, F
    Pas d'uniforme             Uniforme (UK)
```

### 5.2 Vie quotidienne

| En France | Au Royaume-Uni / USA |
|-----------|----------------------|
| On conduit à droite | On conduit à gauche (UK) |
| Euros (€) | Livres (£) ou Dollars ($) |
| Système métrique | Miles, feet, pounds |
| Déjeuner à 12h-13h | Lunch à 12h, léger |
| Dîner à 19h-20h | Dinner à 18h-19h |

### 5.3 Expressions typiques

```
    TYPICAL EXPRESSIONS

    🇬🇧 BRITISH ENGLISH           🇺🇸 AMERICAN ENGLISH
    ──────────────────────────────────────────────
    flat                         apartment
    lift                         elevator
    pavement                     sidewalk
    rubbish                      garbage/trash
    football                     soccer
    biscuit                      cookie
    chips                        fries
    crisps                       chips
    queue                        line
    holiday                      vacation
```

---

## 6. Famous landmarks - Lieux célèbres

### 6.1 Au Royaume-Uni

```
    UK LANDMARKS

    🏰 Big Ben & Houses of Parliament (London)
    🎡 London Eye (London)
    🏰 Buckingham Palace (London)
    🏰 Edinburgh Castle (Scotland)
    🗿 Stonehenge (England)
    🏔️ Loch Ness (Scotland)
```

### 6.2 Aux États-Unis

```
    USA LANDMARKS

    🗽 Statue of Liberty (New York)
    🌉 Golden Gate Bridge (San Francisco)
    🏜️ Grand Canyon (Arizona)
    🗻 Mount Rushmore (South Dakota)
    🎬 Hollywood Sign (Los Angeles)
    🏛️ White House (Washington D.C.)
```

---

## Exercices

### Exercice 1 : Géographie

??? question "Match the country with its capital"
    1. United Kingdom → ___
    2. United States → ___
    3. Australia → ___
    4. Ireland → ___

    ??? success "Answers"
        1. United Kingdom → **London**
        2. United States → **Washington D.C.**
        3. Australia → **Canberra**
        4. Ireland → **Dublin**

### Exercice 2 : Fêtes

??? question "When are these holidays?"
    1. Halloween → ___
    2. Thanksgiving → ___
    3. Independence Day → ___

    ??? success "Answers"
        1. Halloween → **October 31st**
        2. Thanksgiving → **4th Thursday of November**
        3. Independence Day → **July 4th**

### Exercice 3 : British vs American English

??? question "Match British and American words"
    1. flat (UK) = ___ (US)
    2. lift (UK) = ___ (US)
    3. biscuit (UK) = ___ (US)

    ??? success "Answers"
        1. flat = **apartment**
        2. lift = **elevator**
        3. biscuit = **cookie**

---

## Évaluation

??? question "Question 1 : Combien de nations composent le Royaume-Uni ?"

    ??? success "Answer"
        **4** : England, Scotland, Wales, Northern Ireland

??? question "Question 2 : Quelle est la tradition principale de Thanksgiving ?"

    ??? success "Answer"
        Un **repas en famille** avec de la **dinde** (turkey) pour remercier.

??? question "Question 3 : Comment dit-on 'ascenseur' en anglais britannique et américain ?"

    ??? success "Answer"
        UK : **lift** / US : **elevator**

??? question "Question 4 : Que crie-t-on à Halloween en demandant des bonbons ?"

    ??? success "Answer"
        **Trick or treat!**

---

## Bonus : Quiz culturel

!!! example "Test your knowledge!"

    1. What is the name of the British royal palace in London?
    2. What colour is associated with St Patrick's Day?
    3. What do Americans eat at Thanksgiving?
    4. How many stars are on the American flag?
    5. What is the name of the famous clock tower in London?

    ??? success "Answers"
        1. **Buckingham Palace**
        2. **Green**
        3. **Turkey** (and pumpkin pie)
        4. **50 stars** (one for each state)
        5. **Big Ben**

---

## Fiche de révision

!!! abstract "L'essentiel à retenir"

    **PAYS ANGLOPHONES**
    - UK, USA, Canada, Australia, Ireland...
    - UK = England + Scotland + Wales + N. Ireland

    **SYMBOLES**
    - UK : Big Ben, double-decker, tea
    - USA : Statue of Liberty, hamburger

    **FÊTES**
    - Halloween (31 oct) : Trick or treat!
    - Thanksgiving (nov) : turkey, family
    - Christmas (25 déc) : crackers, pudding

    **DIFFÉRENCES UK/US**
    - flat/apartment, lift/elevator
    - football/soccer, biscuit/cookie

---

## Conclusion de la formation

!!! success "Congratulations!"
    Tu as terminé la formation d'anglais 6ème !

    Tu as appris :

    - Te présenter et parler de toi
    - Les temps essentiels (présent simple, BE+ING, prétérit)
    - Du vocabulaire varié (famille, maison, nourriture, loisirs)
    - La culture anglophone

    **Keep practising! The more you use English, the better you'll get!**

---

## Navigation

[⬅️ Module 9 - Le prétérit](module-09-preterit.md){ .md-button }
[🏠 Retour à l'index](index.md){ .md-button .md-button--primary }
