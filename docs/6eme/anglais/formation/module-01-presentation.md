# Module 1 - Se présenter / Introducing yourself

!!! info "Objectifs du module"
    À la fin de ce module, tu sauras :

    - 🎯 Saluer et dire au revoir en anglais
    - 🎯 Te présenter (nom, âge, nationalité)
    - 🎯 Utiliser le verbe BE au présent
    - 🎯 Poser des questions simples

    **Durée estimée : 2-3 heures**

---

## 🎮 In Real Life: English is everywhere!

!!! tip "Tu parles déjà anglais sans le savoir !"

    **Dans les jeux vidéo :**

    | Ce que tu vois | Traduction |
    |----------------|------------|
    | **Start** | Commencer |
    | **Play** | Jouer |
    | **Game Over** | Partie terminée |
    | **Loading** | Chargement |
    | **Settings** | Paramètres |
    | **Level** | Niveau |
    | **New Game** | Nouvelle partie |
    | **Continue** | Continuer |

!!! example "L'anglais sur les réseaux"

    **Tu utilises ces mots tous les jours :**

    | Anglais | Signification |
    |---------|---------------|
    | **Like** | J'aime |
    | **Share** | Partager |
    | **Follow** | Suivre |
    | **Comment** | Commentaire |
    | **Post** | Publication |
    | **Story** | Histoire (éphémère) |
    | **Live** | En direct |
    | **Subscribe** | S'abonner |

!!! warning "Les faux-amis"

    **Attention, ces mots ne veulent pas dire ce que tu crois !**

    | Anglais | ❌ Faux sens | ✅ Vrai sens |
    |---------|-------------|--------------|
    | **actually** | actuellement | en fait |
    | **library** | librairie | bibliothèque |
    | **college** | collège | université |
    | **sensible** | sensible | raisonnable |

??? warning "⚠️ Pièges à éviter : se présenter"

    **Piège 1 : Oublier AM/IS/ARE selon le sujet**

    | Sujet | Verbe BE |
    |-------|----------|
    | I | **am** |
    | You / We / They | **are** |
    | He / She / It | **is** |

    ❌ "I are 12 years old" → FAUX !
    ✅ "I **am** 12 years old"

    ---

    **Piège 2 : Dire "I have 12 years"**

    En anglais, on utilise **BE** (être), pas HAVE (avoir) pour l'âge !

    - ❌ "I **have** 12 years" (traduction mot à mot du français)
    - ✅ "I **am** 12 years old"

    **En anglais, tu "ES" ton âge, tu ne l'"AS" pas !**

    ---

    **Piège 3 : Confondre les contractions**

    | Forme complète | Contraction |
    |----------------|-------------|
    | I am | I'm |
    | You are | You're |
    | He is | He's |

    ❌ "Im" (sans apostrophe) ou "your" (possessif) ≠ "you're" (you are)

    ---

    **Piège 4 : Les majuscules des nationalités**

    - ❌ "I am french"
    - ✅ "I am **F**rench"

    **En anglais, nationalités et langues = MAJUSCULE !**

!!! example "Se présenter comme dans un jeu"

    **Créer son profil de joueur = se présenter !**

    ```
    Username: Théo_Gaming
    Name: Théo
    Age: 12
    Country: France
    Favorite game: Minecraft
    ```

    **En anglais :**
    - My name is Théo.
    - I am 12 years old.
    - I am French.
    - I like playing Minecraft.

---

## 1. Les salutations - Greetings

### 1.1 Dire bonjour

```
    GREETINGS (Salutations)

    ☀️ MATIN           ☀️ APRÈS-MIDI       🌙 SOIR/NUIT
    Good morning!      Good afternoon!     Good evening!
    (avant midi)       (12h-18h)           (après 18h)

    TOUTE LA JOURNÉE
    ─────────────────
    Hello!             Hi!                 Hey!
    (formel)           (courant)           (familier)
```

??? abstract "🔄 Autre façon de comprendre : les salutations"
    **Les salutations, c'est comme les niveaux de familiarité dans un jeu !**

    | Niveau | Anglais | Quand ? |
    |--------|---------|---------|
    | 👔 Formel | "Good morning/afternoon/evening" | Prof, adulte inconnu |
    | 😊 Courant | "Hello!" | La plupart des situations |
    | 🎮 Familier | "Hi!" "Hey!" | Amis, famille |

    ---

    **L'astuce du moment de la journée :**
    - **Morning** = matin (avant midi)
    - **Afternoon** = après-midi (12h-18h)
    - **Evening** = soirée (après 18h)
    - **Night** = nuit (quand on va dormir)

    "Good night" = bonne nuit (pour aller dormir), pas "bonsoir" !

### 1.2 Dire au revoir

| Anglais | Français | Utilisation |
|---------|----------|-------------|
| **Goodbye** | Au revoir | Formel |
| **Bye!** | Salut ! | Courant |
| **See you!** | À plus ! | Familier |
| **See you later** | À plus tard | Courant |
| **See you tomorrow** | À demain | Courant |
| **Good night** | Bonne nuit | Le soir |

### 1.3 Comment ça va ?

```
    DEMANDER                    RÉPONDRE

    How are you?            →   I'm fine, thank you.
    (Comment allez-vous ?)      (Je vais bien, merci.)

    How are you doing?      →   I'm OK / I'm great!
    (Comment tu vas ?)          (Ça va / Super !)

    What's up?              →   Not much. / Nothing special.
    (Quoi de neuf ?)            (Pas grand-chose.)
```

!!! tip "Politesse"
    En anglais, on répond puis on demande à l'autre :
    "I'm fine, thank you. **And you?**"

---

## 2. Se présenter - Introducing yourself

### 2.1 Le vocabulaire de base

```
    SE PRÉSENTER

    👤 NOM
    My name is Tom.        →  Je m'appelle Tom.
    I'm Emma.              →  Je suis Emma.

    🎂 ÂGE
    I am 11 years old.     →  J'ai 11 ans.
    I'm twelve.            →  J'ai 12 ans.

    🏠 ORIGINE
    I'm from France.       →  Je viens de France.
    I live in Paris.       →  J'habite à Paris.

    🇫🇷 NATIONALITÉ
    I'm French.            →  Je suis français(e).
```

### 2.2 Exemple de présentation

!!! quote "Hello!"
    My name is Lucas.
    I am 11 years old.
    I'm from France.
    I live in Lyon.
    I'm French.
    Nice to meet you!

---

## 3. Le verbe BE (être) au présent

### 3.1 Conjugaison

!!! success "À retenir"
    Le verbe **BE** a 3 formes au présent : **AM, IS, ARE**

```
    CONJUGAISON DE BE

    I           AM      →  I am a student.
    You         ARE     →  You are nice.
    He/She/It   IS      →  He is tall.
    We          ARE     →  We are friends.
    They        ARE     →  They are French.
```

### 3.2 Formes contractées

| Forme complète | Forme contractée |
|----------------|------------------|
| I am | I'm |
| You are | You're |
| He is | He's |
| She is | She's |
| It is | It's |
| We are | We're |
| They are | They're |

!!! tip "À l'oral"
    On utilise presque toujours les formes contractées à l'oral :
    "I'm fine" plutôt que "I am fine"

### 3.3 Forme négative

```
    NÉGATION : BE + NOT

    I am not        →  I'm not         →  Je ne suis pas
    You are not     →  You aren't      →  Tu n'es pas
    He is not       →  He isn't        →  Il n'est pas
    She is not      →  She isn't       →  Elle n'est pas
    We are not      →  We aren't       →  Nous ne sommes pas
    They are not    →  They aren't     →  Ils ne sont pas

    Exemples :
    I'm not English. (Je ne suis pas anglais.)
    She isn't 12. (Elle n'a pas 12 ans.)
```

### 3.4 Forme interrogative

```
    QUESTION : BE en début de phrase

    Am I...?        →  Suis-je... ?
    Are you...?     →  Es-tu... ?
    Is he/she...?   →  Est-il/elle... ?
    Are we...?      →  Sommes-nous... ?
    Are they...?    →  Sont-ils... ?

    Exemples :
    Are you French?     →  Es-tu français ?
    Is she your sister? →  Est-elle ta soeur ?
```

---

## 4. Poser des questions

### 4.1 Les mots interrogatifs

```
    QUESTION WORDS (Mots interrogatifs)

    What?       →  Quoi ? / Quel ?
    Who?        →  Qui ?
    Where?      →  Où ?
    When?       →  Quand ?
    Why?        →  Pourquoi ?
    How?        →  Comment ?
    How old?    →  Quel âge ?
```

### 4.2 Questions courantes

| Question | Traduction |
|----------|------------|
| What's your name? | Comment t'appelles-tu ? |
| How old are you? | Quel âge as-tu ? |
| Where are you from? | D'où viens-tu ? |
| Where do you live? | Où habites-tu ? |
| Who is this? | Qui est-ce ? |

---

## 5. Les pays et nationalités

### 5.1 Tableau des nationalités

| Pays (Country) | Nationalité (Nationality) |
|----------------|--------------------------|
| France | French |
| England | English |
| United Kingdom | British |
| United States | American |
| Spain | Spanish |
| Germany | German |
| Italy | Italian |
| China | Chinese |
| Japan | Japanese |

!!! warning "Attention"
    Les nationalités prennent toujours une **majuscule** en anglais !

---

## Exercices

### Exercice 1 : Complète avec AM, IS ou ARE

??? question "Complete the sentences"
    1. I ___ a student.
    2. She ___ my friend.
    3. We ___ in the classroom.
    4. It ___ a cat.
    5. They ___ English.

    ??? success "Answers"
        1. I **am** a student.
        2. She **is** my friend.
        3. We **are** in the classroom.
        4. It **is** a cat.
        5. They **are** English.

### Exercice 2 : Mets à la forme négative

??? question "Make negative sentences"
    1. I am 13 years old. → I ___ 13 years old.
    2. He is American. → He ___ American.
    3. They are at school. → They ___ at school.

    ??? success "Answers"
        1. I **am not / I'm not** 13 years old.
        2. He **is not / isn't** American.
        3. They **are not / aren't** at school.

### Exercice 3 : Pose la question

??? question "Write the question"
    1. ___ your name? - My name is Sophie.
    2. ___ are you from? - I'm from France.
    3. ___ old are you? - I'm 11.

    ??? success "Answers"
        1. **What's** your name?
        2. **Where** are you from?
        3. **How** old are you?

---

## Évaluation

??? question "Question 1 : Comment dit-on 'Bonjour' le matin ?"

    ??? success "Answer"
        **Good morning**

??? question "Question 2 : Complete: She ___ French."

    ??? success "Answer"
        She **is** French.

??? question "Question 3 : Comment demande-t-on l'âge de quelqu'un ?"

    ??? success "Answer"
        **How old are you?**

??? question "Question 4 : Traduis : 'Je m'appelle Tom et j'ai 12 ans.'"

    ??? success "Answer"
        **My name is Tom and I am 12 years old.** / **I'm Tom and I'm 12.**

---

## Bonus : Mini-dialogue

!!! example "Practice this dialogue"

    **A:** Hello! What's your name?

    **B:** Hi! My name is Emma. And you?

    **A:** I'm Lucas. Nice to meet you!

    **B:** Nice to meet you too. Where are you from?

    **A:** I'm from France. I live in Paris. And you?

    **B:** I'm from England. I live in London.

    **A:** How old are you?

    **B:** I'm 11. And you?

    **A:** I'm 12. See you later!

    **B:** Bye!

---

## Fiche de révision

!!! abstract "L'essentiel à retenir"

    **GREETINGS**
    - Hello / Hi / Good morning / Good afternoon
    - Goodbye / Bye / See you

    **BE AU PRÉSENT**
    - I am / You are / He-She-It is / We-They are
    - Négation : am not, isn't, aren't
    - Question : Are you...? Is he...?

    **SE PRÉSENTER**
    - My name is... / I'm...
    - I am ... years old
    - I'm from... / I live in...

    **MOTS INTERROGATIFS**
    - What, Who, Where, When, Why, How

---

## Navigation

[⬅️ Module 0 - Test initial](module-00-depart.md){ .md-button }
[➡️ Module 2 - Nombres et heure](module-02-nombres-heure.md){ .md-button .md-button--primary }
