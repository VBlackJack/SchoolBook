# Module 6 - La cellule

!!! info "Objectifs du module"
    À la fin de ce module, tu sauras :

    - 🎯 Définir ce qu'est une cellule
    - 🎯 Identifier les constituants d'une cellule
    - 🎯 Comparer cellule animale et cellule végétale
    - 🎯 Comprendre que les êtres vivants sont constitués de cellules

    **Durée estimée : 2-3 heures**

---

## 🎮 Dans la vraie vie : les cellules, briques du vivant

!!! tip "Tu es fait de cellules !"

    **Ton corps en chiffres :**

    - ~37 000 milliards de cellules
    - 200 types de cellules différentes
    - Chaque seconde, des millions de cellules meurent et naissent

    **Comparaison :**

    - 37 000 milliards = si tu comptais 1 cellule par seconde, il faudrait plus d'1 million d'années !

!!! example "Les cellules, c'est comme des briques LEGO"

    **Analogie avec la construction :**

    | LEGO | Cellules |
    |------|----------|
    | 1 brique | 1 cellule |
    | Plusieurs briques assemblées | Un tissu (peau, muscle...) |
    | Une construction entière | Un organe (cœur, cerveau...) |
    | Plusieurs constructions | Un organisme (toi !) |

    **Dans Minecraft aussi :**

    - 1 bloc = 1 cellule
    - Plusieurs blocs = une structure
    - Le monde entier = un organisme géant !

!!! warning "Taille des cellules"

    **Tu ne peux pas voir une cellule à l'œil nu !**

    | Objet | Taille |
    |-------|--------|
    | Cheveu | 100 μm d'épaisseur |
    | Cellule humaine | 10-100 μm |
    | Bactérie | 1-10 μm |
    | Virus | 0,02-0,3 μm (ultra petit !) |

    **1 mm = 1000 μm (micromètres)**

    Sur 1 mm, tu peux aligner ~10 cellules !

!!! example "Cellule animale vs cellule végétale"

    **La grande différence :**

    | Cellule animale | Cellule végétale |
    |-----------------|------------------|
    | Pas de paroi rigide | Paroi cellulosique (rigide) |
    | Pas de chloroplastes | Chloroplastes (photosynthèse) |
    | Forme variable | Forme rectangulaire |

    **C'est pour ça que les plantes sont rigides et ne bougent pas !**

---

## 1. Qu'est-ce qu'une cellule ?

### 1.1 Définition

!!! success "À retenir"
    La **cellule** est l'unité de base de tous les êtres vivants. C'est la plus petite structure capable de vivre de façon autonome.

```
    LA CELLULE : UNITÉ DU VIVANT

    Tous les êtres vivants sont constitués de cellules :

    🦠 BACTÉRIE         1 cellule (unicellulaire)
    🌿 PLANTE           Des milliards de cellules
    🐕 CHIEN            Des milliers de milliards
    🧑 HUMAIN           ~37 000 milliards de cellules !
```

### 1.2 Observer les cellules

Les cellules sont **microscopiques** (invisibles à l'œil nu). On les observe avec un **microscope**.

Taille moyenne d'une cellule : 10 à 100 micromètres (μm)
1 mm = 1000 μm

---

## 2. Constituants d'une cellule

### 2.1 Structure générale

!!! success "À retenir"
    Toutes les cellules possèdent :
    - Une **membrane plasmique** (enveloppe)
    - Un **cytoplasme** (liquide intérieur)
    - Un **noyau** (contient l'ADN)

```
    SCHÉMA D'UNE CELLULE ANIMALE

    ┌────────────────────────────────────┐
    │          Membrane plasmique        │
    │  ┌──────────────────────────────┐  │
    │  │                              │  │
    │  │      Cytoplasme              │  │
    │  │                              │  │
    │  │      ┌────────┐              │  │
    │  │      │ Noyau  │              │  │
    │  │      │ (ADN)  │              │  │
    │  │      └────────┘              │  │
    │  │                              │  │
    │  └──────────────────────────────┘  │
    └────────────────────────────────────┘
```

### 2.2 Rôle de chaque constituant

| Élément | Rôle |
|---------|------|
| **Membrane** | Protège la cellule, contrôle les échanges |
| **Cytoplasme** | Milieu où se déroulent les réactions chimiques |
| **Noyau** | Contient l'information génétique (ADN) |

---

## 3. Cellule animale et cellule végétale

### 3.1 Comparaison

!!! success "À retenir"
    La cellule végétale possède des éléments supplémentaires :
    - Une **paroi** rigide (cellulose)
    - Des **chloroplastes** (photosynthèse)
    - Une grande **vacuole**

```
    CELLULE ANIMALE             CELLULE VÉGÉTALE

    ┌──────────────┐           ═══════════════════ Paroi
    │              │           ║                 ║
    │  ┌────┐      │           ║  ┌────┐   🟢   ║ Chloroplaste
    │  │Noyau│     │           ║  │Noyau│        ║
    │  └────┘      │           ║  └────┘        ║
    │              │           ║  ┌──────────┐  ║
    │              │           ║  │ Vacuole  │  ║
    │              │           ║  └──────────┘  ║
    └──────────────┘           ═══════════════════

    • Membrane                 • Paroi + Membrane
    • Pas de paroi             • Chloroplastes (verts)
    • Pas de chloroplastes     • Grande vacuole
```

### 3.2 Tableau comparatif

| Élément | Cellule animale | Cellule végétale |
|---------|:---------------:|:----------------:|
| Membrane | ✅ | ✅ |
| Cytoplasme | ✅ | ✅ |
| Noyau | ✅ | ✅ |
| Paroi | ❌ | ✅ |
| Chloroplastes | ❌ | ✅ |
| Grande vacuole | ❌ | ✅ |

---

## 4. Unicellulaires et pluricellulaires

### 4.1 Définitions

| Type | Définition | Exemples |
|------|------------|----------|
| **Unicellulaire** | Une seule cellule | Bactérie, amibe, levure |
| **Pluricellulaire** | Plusieurs cellules | Plantes, animaux, champignons |

### 4.2 Des cellules spécialisées

Chez les êtres pluricellulaires, les cellules se **spécialisent** :
- Cellules musculaires (contraction)
- Cellules nerveuses (transmission)
- Cellules de la peau (protection)
- Cellules du sang (transport)

---

## Exercices

??? question "Exercice 1 : Légende ce schéma de cellule"
    Membrane, cytoplasme, noyau

    ??? success "Correction"
        - Enveloppe externe : **membrane**
        - Liquide intérieur : **cytoplasme**
        - Structure centrale : **noyau**

??? question "Exercice 2 : Cellule animale ou végétale ?"
    1. Possède une paroi rigide
    2. Contient des chloroplastes
    3. N'a pas de grande vacuole

    ??? success "Correction"
        1. **Végétale**
        2. **Végétale**
        3. **Animale**

---

## Évaluation

??? question "Question 1 : Qu'est-ce qu'une cellule ?"

    ??? success "Réponse"
        L'**unité de base** de tous les êtres vivants

??? question "Question 2 : Cite les 3 constituants communs à toutes les cellules."

    ??? success "Réponse"
        **Membrane, cytoplasme, noyau**

??? question "Question 3 : Quel élément permet la photosynthèse dans les cellules végétales ?"

    ??? success "Réponse"
        Les **chloroplastes**

??? question "Question 4 : Un être unicellulaire est composé de combien de cellules ?"

    ??? success "Réponse"
        **Une seule cellule**

---

## Fiche de révision

!!! abstract "L'essentiel à retenir"

    **DÉFINITION**
    - La cellule est l'**unité de base** du vivant

    **CONSTITUANTS COMMUNS**
    - **Membrane** : enveloppe
    - **Cytoplasme** : liquide intérieur
    - **Noyau** : contient l'ADN

    **CELLULE VÉGÉTALE EN PLUS**
    - **Paroi** rigide
    - **Chloroplastes** (photosynthèse)
    - Grande **vacuole**

---

## Navigation

[⬅️ Module 5 - Les êtres vivants](module-05-vivant.md){ .md-button }
[➡️ Module 7 - Nutrition](module-07-nutrition.md){ .md-button .md-button--primary }
