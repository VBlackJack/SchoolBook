# Module 10 - L'environnement et l'action de l'Homme

!!! info "Objectifs du module"
    À la fin de ce module, tu sauras :

    - 🎯 Définir un écosystème et ses composants
    - 🎯 Expliquer les relations entre les êtres vivants
    - 🎯 Identifier les impacts de l'Homme sur l'environnement
    - 🎯 Proposer des actions pour protéger la planète

    **Durée estimée : 2-3 heures**

---

## 🎮 Dans la vraie vie : protéger la planète

!!! tip "L'environnement, ça te concerne !"

    **Tu peux agir tous les jours :**

    | Action | Impact |
    |--------|--------|
    | Éteindre les lumières | Économie d'énergie |
    | Prendre une douche courte | Économie d'eau |
    | Trier ses déchets | Recyclage |
    | Aller à pied/vélo | Moins de pollution |
    | Manger local/de saison | Moins de transport |

    **Chaque petit geste compte quand on est 8 milliards !**

!!! example "L'environnement dans les jeux vidéo"

    **Minecraft = un écosystème simplifié :**

    - Biomes différents (forêt, désert, océan)
    - Chaînes alimentaires (loups mangent les moutons)
    - Ressources limitées (minerais épuisables)

    **Autres exemples :**

    - **Animal Crossing** → Gestion d'une île, saisons
    - **Eco** → Jeu sur l'équilibre écologique
    - **Subnautica** → Écosystème sous-marin

!!! warning "Les menaces sur la planète"

    **Ce qui met en danger l'environnement :**

    | Problème | Cause | Conséquence |
    |----------|-------|-------------|
    | Réchauffement climatique | CO₂, gaz à effet de serre | Fonte des glaces, canicules |
    | Pollution plastique | Déchets mal gérés | Océans pollués |
    | Déforestation | Agriculture intensive | Perte de biodiversité |
    | Extinction d'espèces | Chasse, destruction habitat | Disparition définitive |

    **1 million d'espèces menacées d'extinction !**

??? warning "⚠️ Pièges à éviter : environnement"

    **Piège 1 : Confondre biotope et biocénose**

    | Biotope | Biocénose |
    |---------|-----------|
    | Le **milieu** de vie | Les **êtres vivants** |
    | Sol, eau, climat, lumière | Animaux, plantes, champignons |
    | "Le décor" | "Les acteurs" |

    **Écosystème = Biotope + Biocénose**

    ---

    **Piège 2 : Oublier que la chaîne alimentaire commence par un végétal**

    - ❌ Lapin → Herbe → Renard
    - ✅ **Herbe** → Lapin → Renard

    Les **producteurs** (végétaux) sont TOUJOURS au début car ils fabriquent leur nourriture !

    ---

    **Piège 3 : Confondre producteur et consommateur**

    | Producteur | Consommateur |
    |------------|--------------|
    | Végétaux | Animaux |
    | Fabrique sa nourriture | Mange d'autres êtres |
    | Autotrophe | Hétérotrophe |

    **Les décomposeurs recyclent les restes !**

    ---

    **Piège 4 : Mal comprendre l'effet de serre**

    - ❌ "L'effet de serre c'est mal"
    - ✅ L'effet de serre **naturel** est nécessaire (sinon -18°C sur Terre !)

    Le problème, c'est l'**excès** de gaz à effet de serre (CO₂) qui amplifie le phénomène.

    ---

    **Piège 5 : Confondre les 3 piliers du développement durable**

    | Pilier | Objectif |
    |--------|----------|
    | **Écologique** | Protéger l'environnement |
    | **Social** | Bien-être de tous |
    | **Économique** | Prospérité partagée |

    **Les 3 doivent être en équilibre !**

    ---

    **Piège 6 : Mélanger les 5 R**

    Dans l'ordre de priorité :

    1. **R**efuser (le mieux = ne pas acheter)
    2. **R**éduire (consommer moins)
    3. **R**éutiliser (donner, réparer)
    4. **R**ecycler (trier ses déchets)
    5. **R**endre à la terre (composter)

    **Le meilleur déchet = celui qu'on ne produit pas !**

!!! example "Chaînes alimentaires du quotidien"

    **Qui mange qui ?**

    ```
    Herbe → Lapin → Renard → Décomposeurs
       ↑                          │
       └──────────────────────────┘
              (cycle fermé)
    ```

    **Si on supprime un maillon :**

    - Plus de renards → Trop de lapins → Plus d'herbe → Plus de lapins

    **C'est pour ça qu'il faut protéger TOUTES les espèces !**

!!! tip "Les écogestes"

    **Règle des 5R :**

    1. **R**efuser (ce dont on n'a pas besoin)
    2. **R**éduire (sa consommation)
    3. **R**éutiliser (plutôt que jeter)
    4. **R**ecycler (quand c'est possible)
    5. **R**endre à la terre (composter)

    **Le meilleur déchet = celui qu'on ne produit pas !**

---

## 1. Les écosystèmes

### 1.1 Qu'est-ce qu'un écosystème ?

!!! success "À retenir"
    Un **écosystème** est l'ensemble formé par :
    - Un **milieu de vie** (biotope) : sol, eau, air, climat
    - Les **êtres vivants** (biocénose) : animaux, végétaux, micro-organismes
    - Les **relations** entre eux

??? abstract "🔄 Autre façon de comprendre : l'écosystème"
    **Un écosystème, c'est comme un serveur Minecraft !**

    - **Biotope** = le décor, le terrain (biome forêt, désert, océan...)
    - **Biocénose** = tous les mobs et joueurs
    - **Relations** = qui mange qui, qui aide qui

    Si tu supprimes tous les loups → trop de moutons → plus d'herbe → mort des moutons !

    ---

    **La chaîne alimentaire = qui mange qui**

    ```
    PRODUCTEURS → CONSOMMATEURS 1 → CONSOMMATEURS 2 → DÉCOMPOSEURS
    (plantes)      (herbivores)      (carnivores)      (bactéries)

    Herbe    →      Lapin      →      Renard      →    Champignons
    ```

    **L'énergie passe du Soleil aux plantes, puis de mangeur en mangeur !**

    ---

    **La règle des 10% :**

    À chaque niveau, 90% de l'énergie est perdue :
    - Pour faire 1 kg de renard, il faut ~10 kg de lapins
    - Pour faire 10 kg de lapins, il faut ~100 kg d'herbe

    **C'est pour ça qu'il y a plus d'herbivores que de carnivores !**

    ---

    **L'impact de l'Homme = on déséquilibre le système**

    - Déforestation → animaux sans maison
    - Pollution → animaux malades
    - Surpêche → plus de poissons
    - CO₂ → réchauffement climatique

    **Mais on peut aussi agir positivement (recycler, protéger, économiser) !**

```
    ÉCOSYSTÈME = BIOTOPE + BIOCÉNOSE

    ┌────────────────────────────────────────────────┐
    │                  ÉCOSYSTÈME                     │
    │                                                 │
    │   BIOTOPE (milieu)      BIOCÉNOSE (vivants)    │
    │   ━━━━━━━━━━━━━━━       ━━━━━━━━━━━━━━━━━━━    │
    │   ☀️ Lumière           🌳 Végétaux            │
    │   🌡️ Température        🐿️ Animaux            │
    │   💧 Eau                🍄 Champignons         │
    │   🪨 Sol                🦠 Bactéries           │
    │                                                 │
    │            ↕️ INTERACTIONS ↕️                   │
    └────────────────────────────────────────────────┘
```

### 1.2 Exemples d'écosystèmes

| Écosystème | Biotope | Biocénose |
|------------|---------|-----------|
| **Forêt** | Sol, humidité, ombre | Arbres, oiseaux, champignons |
| **Étang** | Eau douce, vase | Nénuphars, poissons, grenouilles |
| **Prairie** | Sol, soleil | Herbes, insectes, lapins |
| **Océan** | Eau salée | Algues, poissons, coraux |

---

## 2. Les relations entre êtres vivants

### 2.1 Les chaînes alimentaires

!!! success "À retenir"
    Une **chaîne alimentaire** montre "qui mange qui" dans un écosystème.
    Elle commence toujours par un **végétal** (producteur).

```
    CHAÎNE ALIMENTAIRE (exemple forêt)

    🌿 Herbe  →  🐛 Chenille  →  🐦 Mésange  →  🦅 Épervier
       │            │               │               │
    PRODUCTEUR   CONSOMMATEUR   CONSOMMATEUR   CONSOMMATEUR
    (végétal)    1er ordre      2ème ordre     3ème ordre
                 (herbivore)    (carnivore)    (carnivore)

    → signifie "est mangé par"
```

### 2.2 Le réseau alimentaire

```
    RÉSEAU ALIMENTAIRE (simplifié)

                    🦅 Rapace
                   ╱    ╲
                  ╱      ╲
           🐦 Oiseau    🐍 Serpent
              ╱  ╲        ╱
             ╱    ╲      ╱
         🐛 Chenille  🐸 Grenouille
             ╲       ╱
              ╲     ╱
            🌿 Végétaux

    Plusieurs chaînes interconnectées = réseau
```

### 2.3 Les décomposeurs

!!! success "À retenir"
    Les **décomposeurs** (bactéries, champignons, vers) transforment la matière organique morte en matière minérale, réutilisable par les végétaux.

```
    LE CYCLE DE LA MATIÈRE

         🌿 Végétaux
            ↑
    Sels minéraux
            ↑
    🍄 DÉCOMPOSEURS ← 💀 Matière morte
    (recyclent tout !)   (feuilles, cadavres)
```

---

## 3. L'impact de l'Homme sur l'environnement

### 3.1 Les activités humaines

```
    IMPACTS NÉGATIFS DE L'HOMME

    🏭 POLLUTION                  🌲 DÉFORESTATION
    ├─ Air (CO2, fumées)          ├─ Perte d'habitat
    ├─ Eau (pesticides)           ├─ Moins d'O2 produit
    └─ Sol (déchets)              └─ Érosion des sols

    🎣 SUREXPLOITATION            🌡️ CHANGEMENT CLIMATIQUE
    ├─ Surpêche                   ├─ Hausse des températures
    ├─ Chasse excessive           ├─ Fonte des glaces
    └─ Déforestation              └─ Événements extrêmes
```

### 3.2 Conséquences sur les écosystèmes

| Impact | Conséquence |
|--------|-------------|
| Pollution de l'eau | Mort des poissons, algues toxiques |
| Déforestation | Extinction d'espèces, inondations |
| Réchauffement | Migration, extinction, maladies |
| Plastique | Animaux marins empoisonnés |

### 3.3 Le réchauffement climatique

!!! warning "Problème majeur"
    L'effet de serre est naturel et nécessaire. Mais les activités humaines (voitures, usines) produisent trop de CO₂, ce qui augmente l'effet de serre et réchauffe la Terre.

```
    L'EFFET DE SERRE

         ☀️ Soleil
          │
          ▼
    ══════════════════  Atmosphère (CO2, CH4...)
          │ ↕ ↕ ↕        Retient la chaleur
          ▼
    ───────────────────  Surface terrestre
          🌡️ +1.5°C depuis 1900

    Naturel = bon (sinon -18°C sur Terre)
    Excessif = problème (fonte glaces, catastrophes)
```

---

## 4. Protéger l'environnement

### 4.1 Les gestes du quotidien

!!! tip "Ce que TU peux faire"
    Chaque geste compte pour protéger la planète !

```
    LES ÉCOGESTES

    ♻️ RÉDUIRE          🔄 RÉUTILISER        ♻️ RECYCLER
    ├─ Moins d'emballages  ├─ Réparer           ├─ Trier les déchets
    ├─ Éteindre lumières   ├─ Donner            ├─ Composter
    └─ Moins de viande     └─ Seconde main      └─ Verre, papier, plastique

    🚲 TRANSPORT        💧 EAU               🌱 NATURE
    ├─ Marcher           ├─ Douche courte      ├─ Ne pas jeter
    ├─ Vélo              ├─ Fermer robinet     ├─ Respecter faune
    └─ Transports        └─ Récupérer pluie    └─ Planter
       en commun
```

### 4.2 La règle des 5 R

| R | Action | Exemple |
|---|--------|---------|
| **Refuser** | Dire non au superflu | Pas de sac plastique |
| **Réduire** | Consommer moins | Moins d'emballages |
| **Réutiliser** | Donner une 2ème vie | Réparer, customiser |
| **Recycler** | Transformer | Tri sélectif |
| **Rendre à la terre** | Composter | Épluchures au jardin |

### 4.3 Actions collectives

- **Création de réserves naturelles** et parcs nationaux
- **Lois** contre la pollution et la déforestation
- **Énergies renouvelables** (solaire, éolien)
- **Agriculture raisonnée** et bio
- **Accords internationaux** (COP, Accord de Paris)

---

## 5. Développement durable

### 5.1 Définition

!!! success "À retenir"
    Le **développement durable** répond aux besoins du présent sans compromettre la capacité des générations futures à répondre aux leurs.

```
    LES 3 PILIERS DU DÉVELOPPEMENT DURABLE

              DÉVELOPPEMENT DURABLE
                      │
         ┌────────────┼────────────┐
         │            │            │
    🌍 ÉCOLOGIE   👥 SOCIAL    💰 ÉCONOMIE
    Protéger      Bien-être    Prospérité
    l'environnement  de tous     partagée

    Les 3 doivent être en ÉQUILIBRE
```

### 5.2 Exemples concrets

| Secteur | Action durable |
|---------|----------------|
| Énergie | Panneaux solaires, éoliennes |
| Transport | Vélo, voiture électrique, train |
| Alimentation | Local, bio, moins de viande |
| Habitat | Isolation, LED, récupération eau |

---

## Exercices

??? question "Exercice 1 : Construis une chaîne alimentaire"
    Place ces éléments dans l'ordre : Renard, Herbe, Lapin

    ??? success "Correction"
        **Herbe → Lapin → Renard**

        (Producteur → Herbivore → Carnivore)

??? question "Exercice 2 : Classe ces actions"
    Plastique à usage unique, covoiturage, jeter par terre, composter, laisser couler l'eau

    ??? success "Correction"
        **Bon pour l'environnement :**
        - Covoiturage, composter

        **Mauvais pour l'environnement :**
        - Plastique à usage unique, jeter par terre, laisser couler l'eau

??? question "Exercice 3 : La règle des 5 R"
    Associe chaque action au bon R :
    1. Donner ses vieux vêtements
    2. Dire non au gobelet en plastique
    3. Mettre le verre dans la bonne poubelle

    ??? success "Correction"
        1. **Réutiliser**
        2. **Refuser**
        3. **Recycler**

---

## Évaluation

??? question "Question 1 : Qu'est-ce qu'un écosystème ?"

    ??? success "Réponse"
        C'est l'ensemble formé par un **milieu de vie** (biotope) et les **êtres vivants** (biocénose) qui y habitent, avec leurs **relations**

??? question "Question 2 : Par quoi commence toujours une chaîne alimentaire ?"

    ??? success "Réponse"
        Par un **végétal** (producteur), car il produit sa propre nourriture

??? question "Question 3 : Cite 3 impacts négatifs de l'Homme sur l'environnement."

    ??? success "Réponse"
        Parmi : **pollution, déforestation, surpêche, réchauffement climatique, déchets plastiques**

??? question "Question 4 : Quels sont les 3 piliers du développement durable ?"

    ??? success "Réponse"
        **Écologique** (environnement), **Social** (bien-être), **Économique** (prospérité)

---

## Fiche de révision

!!! abstract "L'essentiel à retenir"

    **ÉCOSYSTÈME**
    - Biotope (milieu) + Biocénose (vivants)
    - Tout est connecté

    **CHAÎNES ALIMENTAIRES**
    - Producteur → Consommateurs
    - Décomposeurs recyclent la matière

    **IMPACTS HUMAINS**
    - Pollution, déforestation, surexploitation
    - Réchauffement climatique

    **PROTÉGER**
    - Les 5 R : Refuser, Réduire, Réutiliser, Recycler, Rendre à la terre
    - Développement durable : écologie + social + économie

---

## Conclusion de la formation

!!! success "Bravo ! 🎉"
    Tu as terminé la formation de Sciences et Technologie de 6ème !

    Tu as appris :

    - La matière et ses transformations
    - Le monde vivant (cellules, nutrition, classification)
    - La Terre et ses phénomènes
    - L'environnement et comment le protéger

    **Continue à observer le monde qui t'entoure avec curiosité !**

---

## Navigation

[⬅️ Module 9 - Phénomènes naturels](module-09-phenomenes.md){ .md-button }
[🏠 Retour à l'index](index.md){ .md-button .md-button--primary }
